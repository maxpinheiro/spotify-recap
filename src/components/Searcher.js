        ...
        <div className="">
            <div className="flex flex-col justify-self-auto">
                <div className="mx-auto">
                    {songs.map(({name, artist, spotifyId, spotifyUri, imageUrl}, idx) => (
                        <div className="flex items-center justify-between my-3 bg-gray-50" key={idx}>
                            ...
                            <button onClick={() => selectSong(spotifyId, spotifyUri, name, artist, imageUrl)} className="rounded-lg px-3 py-1 bg-green-600 text-green-100">Select Song</button>
                        </div>
                    ))}
                </div>
            </div>
